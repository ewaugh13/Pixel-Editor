...
int GifSave::GifIMax(int l, int r) { return l>r?l:r; }
int GifSave::GifIMin(int l, int r) { return l<r?l:r; }
int GifSave::GifIAbs(int i) { return i<0?-i:i; }

// walks the k-d tree to pick the palette entry for a desired color.
// Takes as in/out parameters the current best color and its error -
// only changes them if it finds a better color in its subtree.
// this is the major hotspot in the code at the moment.
void GifSave::GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b, int& bestInd, int& bestDiff, int treeRoot)
{
...
void GifSave::GifSwapPixels(uint8_t* image, int pixA, int pixB)
{
...
int GifSave::GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotIndex)
{
...
void GifSave::GifPartitionByMedian(uint8_t* image, int left, int right, int com, int neededCenter)
{
    if(left < right-1)
    {
...
void GifSave::GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, bool buildForDither, GifPalette* pal)
{
...
// changed pixels only.
int GifSave::GifPickChangedPixels( const uint8_t* lastFrame, uint8_t* frame, int numPixels )
{
    int numChanged = 0;
...
void GifSave::GifMakePalette( const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, GifPalette* pPal )
{
...
void GifSave::GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    int numPixels = width*height;
...
void GifSave::GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
...
// insert a single bit
void GifSave::GifWriteBit( GifBitStatus& stat, uint32_t bit )
{
    bit = bit & 1;
...
void GifSave::GifWriteChunk( FILE* f, GifBitStatus& stat )
{
...
void GifSave::GifWriteCode( FILE* f, GifBitStatus& stat, uint32_t code, uint32_t length )
{
...
void GifSave::GifWritePalette( const GifPalette* pPal, FILE* f )
{
...
void GifSave::GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
...
bool GifSave::GifBegin( GifWriter* writer, const char* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth, bool dither)
{
#if _MSC_VER >= 1400
    writer->f = 0;
...
bool GifSave::GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth, bool dither)
{
    if(!writer->f) return false;
...
bool GifSave::GifEnd( GifWriter* writer )
{
    if(!writer->f) return false;
